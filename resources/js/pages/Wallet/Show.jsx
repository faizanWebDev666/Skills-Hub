...
export default function Show({ wallet, transactions, balance }) {
    ...
    const handleDepositSubmit = async (e) => {
        ...
        if (method === 'stripe' || method === 'paypal') {
            const endpoint = method === 'stripe' ? '/wallet/deposit/stripe' : '/wallet/deposit/paypal';
            ...
            <div className="flex">
                ...
                <main className="flex-1 min-w-0">
                    <div className="px-4 sm:px-6 lg:px-8 py-8">
                                    ...
                                    {transactions.data.length > 0 ? (
                                        <div className="space-y-4">
                                            {transactions.data.slice(0, 5).map((transaction) => (
                                                <div key={transaction.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-2xl">
                                        ...
                                        <div className="text-center py-12">
                                            ...
                                            <p className="text-gray-500">Your transaction history will appear here once you start using your wallet.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        {activeTab === 'deposit' && (
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
                                <h2 className="text-xl font-bold text-gray-900 mb-6">Deposit Funds</h2>
                                ...
                                <form onSubmit={handleDepositSubmit} className="space-y-6">
                                    <div>
                                        ...
                                        {depositForm.errors.amount && (
                                            <p className="mt-1 text-sm text-red-600">{depositForm.errors.amount}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {depositForm.errors.payment_method && (
                                            <p className="mt-1 text-sm text-red-600">{depositForm.errors.payment_method}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => depositForm.setData('reference', e.target.value)}
                                            className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-100 outline-none transition-all"
                                            placeholder="Transaction reference or note"
                                        />
                                        {depositForm.errors.reference && (
                                            <p className="mt-1 text-sm text-red-600">{depositForm.errors.reference}</p>
                                        )}
                                        {depositError && (
                                    ...
                                    >
                                        {depositForm.processing ? 'Processing...' : 'Deposit Funds'}
                                    </button>
                                </form>
                            </div>
                        )}
                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
                                ...
                                <form onSubmit={handleWithdrawSubmit} className="space-y-6">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-gray-500">Available balance: {formatCurrency(balance)}</p>
                                        {withdrawForm.errors.amount && (
                                            <p className="mt-1 text-sm text-red-600">{withdrawForm.errors.amount}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-2">Payment Method</label>
                                        <select
                                            value={withdrawForm.data.payment_method}
                                            onChange={(e) => withdrawForm.setData('payment_method', e.target.value)}
                                            className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-100 outline-none transition-all"
                                        >
                                            <option value="bank_transfer">Bank Transfer</option>
                                            <option value="paypal">PayPal</option>
                                        ...
                                        {withdrawForm.errors.payment_method && (
                                            <p className="mt-1 text-sm text-red-600">{withdrawForm.errors.payment_method}</p>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={withdrawForm.data.account_details.account_number}
                                                onChange={(e) => withdrawForm.setData('account_details', {
                                                    ...withdrawForm.data.account_details,
                                                ...
                                                required
                                            />
                                            {withdrawForm.errors['account_details.account_number'] && (
                                                <p className="mt-1 text-sm text-red-600">{withdrawForm.errors['account_details.account_number']}</p>
                                            )}
                                        </div>

                                        <div>
                                            <label className="block text-sm font-semibold text-gray-700 mb-2">Routing Number (Optional)</label>
                                            <input
                                                type="text"
                                                value={withdrawForm.data.account_details.routing_number}
                                                onChange={(e) => withdrawForm.setData('account_details', {
                                                    ...withdrawForm.data.account_details,
                                            ...
                                            />
                                            {withdrawForm.errors['account_details.routing_number'] && (
                                                <p className="mt-1 text-sm text-red-600">{withdrawForm.errors['account_details.routing_number']}</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className="w-full px-6 py-3 bg-brand-600 text-white rounded-2xl font-semibold hover:bg-brand-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {withdrawForm.processing ? 'Processing...' : 'Request Withdrawal'}
                                    </button>
                                </form>
                            </div>
                        )}
                        ...
                        {activeTab === 'transfer' && (
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
                                <h2 className="text-xl font-bold text-gray-900 mb-6">Transfer Funds</h2>
                                ...
                                <form onSubmit={handleTransferSubmit} className="space-y-6">
                                    <div>
                                        ...
                                        <input
                                            type="email"
                                            value={transferForm.data.recipient_email}
                                            onChange={(e) => transferForm.setData('recipient_email', e.target.value)}
                                            className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-100 outline-none transition-all"
                                            placeholder="recipient@example.com"
                                            required
                                        />
                                        {transferForm.errors.recipient_email && (
                                            <p className="mt-1 text-sm text-red-600">{transferForm.errors.recipient_email}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {transferForm.errors.amount && (
                                            <p className="mt-1 text-sm text-red-600">{transferForm.errors.amount}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => transferForm.setData('message', e.target.value)}
                                            className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-100 outline-none transition-all"
                                            placeholder="Add a note for the recipient"
                                            rows={4}
                                        />
                                        {transferForm.errors.message && (
                                            <p className="mt-1 text-sm text-red-600">{transferForm.errors.message}</p>
                                        )}
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className="w-full px-6 py-3 bg-brand-600 text-white rounded-2xl font-semibold hover:bg-brand-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {transferForm.processing ? 'Sending...' : 'Send Transfer'}
                                    </button>
                                </form>
                            </div>
                        )}
