...
export default function ChatShow({ conversation, conversations, user }) {
    ...
    const getLastActiveString = (lastActiveDate) => {
        ...
        if (diffInMins < 60) return `Last active ${diffInMins} minute${diffInMins !== 1 ? 's' : ''} ago`;
        ...
        if (diffInDays < 7) return `Last active ${diffInDays} day${diffInDays !== 1 ? 's' : ''} ago`;
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
            ...
            <div className="flex-1 flex overflow-hidden">
                <div className="max-w-[1400px] w-full mx-auto flex h-[calc(100vh-76px)] shadow-sm bg-white overflow-hidden lg:my-6 lg:rounded-3xl lg:border lg:border-gray-200 lg:h-[calc(100vh-120px)]">
                    ...
                    {/* Sidebar */}
                    <div className="hidden lg:flex w-[380px] flex-col bg-white border-r border-gray-100 z-10 flex-shrink-0">
                            ...
                            {convList?.map((conv) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`group flex items-center gap-4 p-3 rounded-2xl transition-all duration-200 relative ${
                                            isActive ? 'bg-brand-50/60 ring-1 ring-brand-500/20' : 'hover:bg-gray-50'
                                        }`}
                                    >
                                        {isActive && (
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                                            <div className="flex items-center justify-between mb-0.5">
                                                <h3 className={`font-semibold text-[15px] truncate pr-2 ${isActive || isUnread ? 'text-gray-900' : 'text-gray-700 group-hover:text-gray-900'}`}>
                                                    {conv.other_user?.name}
                                                </h3>
                                                <span className={`text-[11px] font-medium whitespace-nowrap ${isActive || isUnread ? 'text-brand-600' : 'text-gray-400'}`}>
                                            ...
                                            <div className="flex items-center justify-between gap-2">
                                                <p className={`text-[13px] truncate ${isActive || isUnread ? 'text-gray-900 font-medium' : 'text-gray-500'}`}>
                    ...
                    <div className="flex-1 flex flex-col bg-white relative">
                        ...
                        <div className="p-4 lg:px-8 lg:py-5 border-b border-gray-100 flex items-center justify-between bg-white/95 backdrop-blur z-20 shadow-[0_4px_20px_-15px_rgba(0,0,0,0.1)]">
                            <div className="flex items-center gap-4">
                                <Link href={route('chat.index')} className="lg:hidden p-2 -ml-2 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-colors">
                                ...
                                <div className="relative shrink-0">
                                    <div className="w-12 h-12 lg:w-14 lg:h-14 bg-gradient-to-br from-brand-100 to-brand-50 rounded-full flex items-center justify-center text-brand-700 font-bold text-xl ring-1 ring-black/5 shadow-inner overflow-hidden">
                                        {conversation?.other_user?.avatar ? (
                                            <img src={`/storage/${conversation.other_user.avatar}`} alt={conversation.other_user.name} className="w-full h-full object-cover" />
                                        ) : (
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-0.5">
                                        ...
                                        <span className={`hidden sm:inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${getRoleBadge(conversation?.other_user)}`}>
                                            {getRoleLabel(conversation?.other_user)}
                                        </span>
                                    </div>
                                    <div className={`flex items-center gap-1.5 text-xs font-medium ${onlineUsers.has(conversation?.other_user?.id) ? 'text-green-600' : 'text-gray-400'}`}>
                                        {onlineUsers.has(conversation?.other_user?.id) ? (
                                            <>
                                            ...
                                            <>
                                                ...
                                                {getLastActiveString(conversation?.other_user?.last_active_at)}
                                            </>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center gap-2">
                                <button className="p-2.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-full transition-colors" title="Video Call">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                    </svg>
                                </button>
                                <button className="p-2.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-full transition-colors hidden sm:block" title="Call">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                    </svg>
                                </button>
                                <button className="p-2.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-full transition-colors" title="More Options">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 bg-[#FAF9F6] relative custom-scrollbar">
                            ...
                            <div className="relative z-10 space-y-6">
                                {messages?.map((msg, idx) => {
                                    ...
                                    const showAvatar = !isOwn && (idx === 0 || String(messages[idx - 1]?.user_id ?? messages[idx - 1]?.user?.id ?? messages[idx - 1]?.sender_id ?? messages[idx - 1]?.sender?.id) !== String(messageUserId) || showDateLabel);
                                            ...
                                            <div className={`flex items-end gap-3 ${isOwn ? 'justify-end' : 'justify-start'}`}>
                                                {!isOwn && (
                                                    <div className="shrink-0 w-8 h-8 rounded-full bg-brand-100 flex items-center justify-center text-brand-700 text-xs font-bold overflow-hidden">
                                                        {showAvatar ? (
                                                            conversation?.other_user?.avatar ? (
                                                                <img src={`/storage/${conversation.other_user.avatar}`} alt="avatar" className="w-full h-full object-cover" />
                                                            ) : (
                                                                conversation?.other_user?.name?.charAt(0) || 'U'
                                                            )
                                                        ) : null}
                                                ...
                                                <div className={`max-w-[85%] sm:max-w-[75%] md:max-w-[65%] flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                                                    ...
                                                    `}>
                                                        {msg.attachment && (
                                                            <div className={`mb-2 ${msg.content ? 'pb-3 border-b border-white/20' : ''}`}>
                                                                {msg.attachment_type === 'image' ? (
                                                                    <a href={`/storage/${msg.attachment}`} target="_blank" rel="noreferrer">
                                                                        <img src={`/storage/${msg.attachment}`} alt="attachment" className="max-w-full rounded-xl object-cover" style={{ maxHeight: '300px' }} />
                                                                    </a>
                                                                ) : (
                                                                    <a href={`/storage/${msg.attachment}`} target="_blank" rel="noreferrer" className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${isOwn ? 'bg-black/10 hover:bg-black/20 text-white' : 'bg-gray-50 hover:bg-gray-100 text-gray-800'}`}>
                                                                        ...
                                                                        <span className="truncate max-w-[180px] font-medium text-sm">{msg.attachment_name || 'Download File'}</span>
                                                                    </a>
                                                                )}
                                                            </div>
                                                        )}
                                                    ...
                                                    <div className="mt-1.5 flex items-center gap-1 text-[11px] font-medium text-gray-400 px-1">
                                                        ...
                                                        {isOwn && (
                                                            <svg viewBox="0 0 24 24" className="w-4 h-4 text-brand-500 ml-0.5" fill="none" stroke="currentColor" strokeWidth={2}>
                        ...
                        <div className="p-4 lg:p-6 bg-white border-t border-gray-100 z-20">
                            <form onSubmit={handleSubmit} className="flex items-end gap-3 max-w-5xl mx-auto relative">
                                ...
                                {attachment && (
                                    <div className="absolute bottom-full left-0 mb-4 bg-white border border-gray-100 rounded-2xl p-3 shadow-lg shadow-brand-500/5 flex items-center gap-4 z-40 max-w-sm w-full">
                                        {attachment.type.startsWith('image/') ? (
                                            <img src={URL.createObjectURL(attachment)} alt="preview" className="w-14 h-14 rounded-xl object-cover ring-1 ring-gray-100" />
                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500 font-medium mt-0.5">{(attachment.size / 1024 / 1024).toFixed(2)} MB</p>
                                        </div>
                                        <button type="button" onClick={() => setAttachment(null)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors shrink-0">
                                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
