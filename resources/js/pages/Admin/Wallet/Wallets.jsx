                    ...
                    <div className="px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                ...
                                <p className="mt-2 text-gray-600">Search and manage user wallet balances.</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6 mb-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <input
                                    ...
                                    onChange={(e) => router.get('/admin/wallet/wallets', { ...filters, search: e.target.value }, { preserveState: true, preserveScroll: true })}
                                ...
                                <select
                                    ...
                                    onChange={(e) => router.get('/admin/wallet/wallets', { ...filters, order: e.target.value }, { preserveState: true, preserveScroll: true })}
                                    className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-100 outline-none transition-all"
                                >
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {wallets.data.map((wallet) => (
                                            <tr key={wallet.id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{wallet.user?.name}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{wallet.user?.email}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{formatCurrency(wallet.balance)}</td>
