...
export default function VendorReviews({ vendor, reviews, user, sidebarLinks }) {
    ...
    const executeToggle = () => {
        router.post(`/admin/reviews/${vendor.id}/toggle-status`, {}, {
            preserveScroll: true,
                    ...
                    <div className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
                        ...
                        <div className="mb-6 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <Link href="/admin/reviews" className="w-10 h-10 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-50 transition-colors shadow-sm">
                                    <ArrowLeft className="w-5 h-5" />
                                </Link>
                                <div>
                                    <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900">{vendor.name}'s Reviews</h1>
                                    <p className="text-gray-500 mt-1 text-sm">Detailed feedback and gig performance</p>
                                </div>
                            </div>
                            ...
                            >
                                {vendor.banned_at ? 'Activate Vendor' : 'Deactivate Vendor'}
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-black text-gray-900">{avgRating} <span className="text-base font-medium text-gray-500">({vendor.total_reviews} reviews)</span></p>
                                </div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-black text-gray-900">{vendor.delivered_orders_count || 0}</p>
                                </div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                                <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${vendor.banned_at ? 'bg-danger-100 text-danger-600' : 'bg-success-100 text-success-600'}`}>
                                ...
                                <div>
                                    ...
                                    <p className={`text-2xl font-black ${vendor.banned_at ? 'text-danger-600' : 'text-success-600'}`}>
                                        {vendor.banned_at ? 'Deactivated' : 'Active'}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="divide-y divide-gray-100">
                                {reviewsList.length > 0 ? (
                                        ...
                                        <div key={review.id} className="p-6 hover:bg-gray-50 transition-colors">
                                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-3">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className="font-bold text-gray-900">{review.reviewer?.name || 'Unknown User'}</span>
                                                ...
                                                <div className="flex text-amber-400 shrink-0">
                                                    {[...Array(5)].map((_, i) => (
                                                        <Star key={i} className={`w-4 h-4 ${i < review.rating ? 'fill-amber-400' : 'text-gray-200'}`} />
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            {review.reply && (
                                                <div className="mt-4 ml-6 p-4 bg-brand-50 border border-brand-100 rounded-xl relative">
                                                    ...
                                                    <p className="text-sm font-bold text-brand-800 mb-1">Vendor's Reply</p>
                                    ...
                                    <div className="p-12 text-center text-gray-500">
                                        ...
                                        <p className="font-medium">No reviews have been submitted for this vendor yet.</p>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all">
                        <div className="mb-6">
                            <h3 className="text-xl font-bold text-gray-900 mb-2">
                                Confirm {vendor?.banned_at ? 'Activation' : 'Deactivation'}
                            </h3>
                            <p className="text-gray-600">
                                Are you sure you want to {vendor?.banned_at ? 'activate' : 'deactivate'} the vendor <strong>{vendor?.name}</strong>? 
                                {!vendor?.banned_at && " They will no longer be able to log in or receive new orders."}
                            </p>
                        </div>
                        <div className="flex justify-end gap-3">
                            ...
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-bold text-white rounded-xl transition-colors ${
                                    vendor?.banned_at ? 'bg-success-600 hover:bg-success-700' : 'bg-danger-600 hover:bg-danger-700'
                                }`}
                            >
                                Yes, {vendor?.banned_at ? 'Activate' : 'Deactivate'}
                            </button>
                        </div>
                    </div>
                </div>
