...
const StatCard = ({ title, value, subtitle, icon, color, trend }) => {
    const colorClasses = {
        ...
        success:
            "from-success-50 to-success-100 text-success-700 border-success-200",
        warning:
            "from-warning-50 to-warning-100 text-warning-700 border-warning-200",
    ...
    return (
        <div
            className={`bg-linear-to-br ${colorClasses[color]} rounded-2xl p-6 border shadow-sm hover:shadow-md transition-shadow`}
        >
            <div className="flex items-start justify-between">
                <div>
                    ...
                    {trend !== undefined && (
                        <div
                            className={`flex items-center gap-1 mt-2 text-sm font-medium ${trend >= 0 ? "text-success-600" : "text-danger-600"}`}
                        >
                            <svg
...
const ActivityItem = ({ icon, title, time, color }) => {
    ...
    return (
        <div className="flex items-center gap-3 py-3">
            <div
                className={`w-10 h-10 rounded-full ${colorMap[color]} flex items-center justify-center shrink-0`}
            >
                <svg
                    className="w-5 h-5"
                    fill="none"
                        ...
                        <div className="mt-8">
                            <div className="bg-linear-to-br from-brand-600 to-brand-800 rounded-2xl shadow-lg p-6 sm:p-8 text-white">
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                                    <Link
                                        href="/admin/users"
                                        className="group bg-white/10 hover:bg-white/20 rounded-xl p-4 text-center transition-all duration-300 backdrop-blur-sm"
                                    >
                                        <svg
                                    ...
                                    {recentGigs?.length > 0 ? (
                                        recentGigs.map((gig) => (
                                            ...
                                            >
                                                ...
                                                <span
                                                    className={`w-2 h-2 rounded-full shrink-0 ${gig.active ? "bg-success-500" : "bg-gray-400"}`}
                                                ></span>
                                            </div>
                                        ))
                                    ) : (
