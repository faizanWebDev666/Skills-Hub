...
export default function Index({ gigs, user, filters = {}, wishlistGigIds = [] }) {
    ...
    const applyFilters = useCallback((overrides = {}) => {
        ...
        if (params.sort && params.sort !== 'recommended') cleanParams.sort = params.sort;
                        ...
                        <div className="lg:sticky top-24 space-y-8">
                            <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6">
                                <div className="flex items-center justify-between mb-4 sm:mb-6">
                                    <h3 className="font-bold text-base sm:text-lg text-gray-900">Filters</h3>
                                ...
                                <div className="mb-8">
                                    <h4 className="font-semibold text-gray-900 mb-4">Delivery Time</h4>
                                ...
                                <div className="mb-8">
                                    <h4 className="font-semibold text-gray-900 mb-4">Seller Level</h4>
                                    <div className="space-y-3">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900">
                                    {selectedCategory === 'all' ? 'All Services' : `${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1).replace('_', ' ')}`}
                                </h2>
                                        ...
                                        {gig.category && (
                                            <div className="absolute bottom-3 left-3">
                                                <span className="bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium text-gray-700 shadow-sm">
                                                    {gig.category.charAt(0).toUpperCase() + gig.category.slice(1).replace('_', ' ')}
                                                </span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="p-3 sm:p-5">
                                        <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                                            ...
                                            <div className="min-w-0">
                                                <p className="font-semibold text-gray-900 text-xs sm:text-sm truncate">{gig.user?.name || 'Service Provider'}</p>
                                                <div className="flex items-center gap-1">
                                                    <span className="text-brand-500 text-xs">✓</span>
                                                    <span className="text-xs text-gray-500 hidden sm:inline">Level 2 Seller</span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        {(!gigs.data || gigs.data.length === 0) && (
                            <div className="text-center py-20 bg-gray-50 rounded-2xl border border-gray-200">
                                ...
                                <p className="text-gray-600 mb-8 max-w-md mx-auto">
                                    Try adjusting your search or filters. Be the first to create a gig in this category!
                                </p>
                                <Link
                                    href="/gigs/create"
                                ...
                                <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-2">
                                    {gigs.links.map((link, index) => (
                                        <Link
                                            key={index}
                                            href={link.url || '#'}
                                            className={`px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-semibold text-sm sm:text-base transition-all ${
                                                link.active
