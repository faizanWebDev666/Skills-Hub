    ...
    useEffect(() => {
        ...
        const startPolling = () => {
            ...
            interval = setInterval(() => {
                ...
                // We use function state check or just check document state here. But since closure has old isOpen,
                // we should rely on a simpler approach or fetch UnreadCount only.
                // Wait, since we are inside useEffect with [] dependency, `isOpen` will always be false in closure.
                // It's better to omit fetchNotifications from here and just let fetchUnreadCount run.
                // Or use a ref for isOpen. Let's keep it simple for now and fetchUnreadCount only.
                fetchUnreadCount();
        ...
        if (window.Echo && window.userId) {
            window.Echo.private(`user.${window.userId}.notifications`).listen(
            ...
            // No Echo configured, start polling immediately
            setUsePolling(true);
            startPolling();
        }
                                ...
                                <div className="divide-y divide-gray-100">
                                    {notifications.map((notification) => (
                                        <button
                                            ...
                                            className={`w-full px-4 py-3 text-left transition-all hover:bg-cream-50 ${!notification.read
